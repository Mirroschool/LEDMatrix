...
const uint8_t fontGlyphs[][5] PROGMEM = {
  ...
  // Ukrainian chars
  {0x3E, 0x49, 0x49, 0x41, 0x22}, //Є 170 159
  {0x38, 0x54, 0x54, 0x44, 0x28}, //є 186 160

  {0x00, 0x41, 0x7e, 0x41, 0x00}, //Ї 175 161
  {0x00, 0x45, 0x7c, 0x41, 0x00}, //ї 191 162

  {0x7e, 0x02, 0x02, 0x02, 0x03}, //Ґ 165 163
  {0x7c, 0x04, 0x04, 0x04, 0x06}, //ґ 180 164
};

uint8_t getGlyphIndexByCodePoint(uint8_t codePoint){
  uint8_t glyphIndex = 0;

  if ((32 <= codePoint) && (codePoint <= 126)){
    glyphIndex = codePoint-32;
  } else if ((192 <= codePoint) && (codePoint <= 255)){
    glyphIndex = codePoint-97;
  } else {
    switch (codePoint){
      case 170: // є
        glyphIndex = 159;
