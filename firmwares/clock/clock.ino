...
class CreepingLine : public Activity{
    ...
    void setText(uint8_t creepingText[], uint8_t textLength){
      ...
      for(uint8_t i=0; i<textLength; i++){
        ...
        for(uint8_t j=0; j<5; j++){
          uint8_t glyphLine = pgm_read_byte(&fontGlyphs[getGlyphIndexByCodePoint(codePoint)][j]);
    ...
    void render(){
      if (millis() - m_fpsTimer > 1000/m_fps){
        ...
        for (uint8_t i=0; i<MATRIX_WIDTH; i++){
          drawGlyphLine(m_linesBuffer[MATRIX_WIDTH+m_offset-i], MATRIX_WIDTH-i-1, m_offsetY);
        }
  ...
  private:
    uint8_t m_linesBuffer[MATRIX_HEIGHT*MATRIX_WIDTH*5 + 255 + MATRIX_WIDTH*2];
    uint8_t m_offsetY = 4;
    uint8_t m_fps = 14;
...
void onSettings(AsyncWebServerRequest *request) {
  ...
  sprintf(
    responseBuf,
    "{\"modes\":[\"clock\", \"stopwatch\", \"creepingLine\"],\"width\":\"%d\",\"height\":\"%d\"}", // format
    MATRIX_WIDTH,
    MATRIX_HEIGHT
  );
